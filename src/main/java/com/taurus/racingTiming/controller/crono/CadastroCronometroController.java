...
@Component
public class CadastroCronometroController extends FXMLBaseController {
    
    private static final Log LOG = LogFactory.getLog(CadastroCronometroController.class);
    private static final int ITENS_POR_PAGINA = 15;
    
    @Autowired
    private ICronometroController cronometroController;
    @Autowired
    private ProvaDemoController provaDemoController;
    
    private ProvaDataSource dataSource;
    
    public CadastroCronometroController() {
        super();
    }
    ...
    @Value("${fxml.cadastroCronometro.view}")
    @Override
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        tabProva.setRowFactory(new TableViewDuploClique<GenericLinhaView<Prova>>() {
            ...
            public GenericLinhaView eventoDuploClique(GenericLinhaView<Prova> linhaView) {
                abrirCronometragem(CadastroCronometroController.this.dataSource.getBean(linhaView));
                return linhaView;
            }
        });
    }
    ...
    private void abrirCronometragem(Prova prova) {
        this.cronometroController.setProva(prova);
        super.cj.abrirJanela(ListaConstantes.Stages.CRONOMETRAGEM.ordinal(), null, ICronometroController.class, "t_cronometragem", true);
    ...
    private TableView paginar(int indexPagina) {
        try {
            ...
            List<StatusProva> listaStatus = GeralUtil.criarLista(StatusProva.INSCRICAO_FECHADA, StatusProva.CRONOMETRANDO_PARCIAL_LARGADA, StatusProva.CRONOMETRANDO_TODAS_LARGADAS);
    ...
    @FXML
    public void abrirDemo(ActionEvent actionEvent) {
        super.cj.abrirJanela(ListaConstantes.Stages.PROVA_DEMO.ordinal(), ListaConstantes.Stages.CADASTRO_CRONOMETRAGEM.ordinal(), ProvaDemoController.class, "Criar Prova Demo", true);
        Prova prova = provaDemoController.getProva();
        if (prova != null) {
            this.abrirCronometragem(prova);
        }
    }
}
